...
class SAMRoadplus(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        if self.is_dinov2:
            ...
            # Project 768 -> 256 to match decoder, after reshaping patch tokens
    ...
    def forward(self, rgb, graph_points, pairs, valid):
        ...
        if self.is_dinov2:
            # DINOv2 patch size is 14; resize to 518 (=37x14) for clean patch grid
            x_dinov2 = F.interpolate(x, size=(518, 518), mode="bilinear", align_corners=False)
            # Returns [B, 768, 37, 37] with reshape=True
            ...
            if hasattr(outputs, 'last_hidden_state') and len(outputs.last_hidden_state.shape) == 3:
                ...
                # Use the last layer's features (properly normalized)
                image_embeddings = outputs.last_hidden_state
                
                h, w = x.shape[2]//16, x.shape[3]//16
                num_spatial = h * w
                if image_embeddings.shape[1] > num_spatial:
                    # Remove CLS and register tokens (DINOv3 has 1 CLS + 4 registers = 5 extra tokens)
